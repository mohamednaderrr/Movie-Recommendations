...
const Navbarmovies = ({ setMovieType, movieType }) => {
  const [activeLink, setActiveLink] = useState(null);
  const [isNavOpen, setIsNavOpen] = useState(false);

  // Set the active link based on the movieType prop
  useEffect(() => {
    if (movieType) {
      setActiveLink(movieType);
    }
  }, [movieType]);
              ...
              <li key={link.key} className="nav-item w-lg-auto text-center">
                <button
                  className={`btn btn-link d-flex justify-content-center justify-content-lg-start align-items-center gap-3 fw-bold fs-5 ${
                    activeLink === link.key ? 'active-link text-white' : 'text-white'
                  }`}
